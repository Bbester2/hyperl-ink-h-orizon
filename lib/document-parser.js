...
export async function extractLinksFromDocx(buffer) {
    ...
    try {
        ...
        if (commentsFile) {
            ...
            // Extract text content from comments XML (very basic strip tags)
            // Comments usually look like <w:t>Comment text</w:t>
            const commentTextMatches = commentsContent.match(/<w:t[^>]*>(.*?)<\/w:t>/g) || [];
            const commentText = commentTextMatches.map(t => t.replace(/<[^>]+>/g, '')).join(' ');

            const commentMatches = commentText.match(URL_PATTERN) || [];
            for (const url of commentMatches) {
                const cleanUrl = normalizeUrl(url);
                if (cleanUrl) {
                    links.add(cleanUrl);
                    // Add specific context for comments if not already present
                    if (!contexts.has(cleanUrl)) {
                        contexts.set(cleanUrl, `(Found in Comments) ...${url}...`);
                    }
                }
            }
        }

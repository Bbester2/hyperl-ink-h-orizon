            ...
            if (isHtml) {
                try {
                    ...
                    if (finalResponse.bodyUsed || usedMethod === 'GET' || finalResponse.url) {
                        // Check if we can read the body (it might be consumed if we did retries or logic above? actually we haven't consumed it yet in this flow)
                        try {

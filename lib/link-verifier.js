        ...
        if (result.status === 'working' && finalResponse.status === 200) {
            ...
            try {
                ...
                if (contentType.includes('text/html')) {
                    ...
                    // 2. Try Meta Tags for Date (if not found in header or we prefer semantic date)
                    // Common patterns: article:published_time, article:modified_time, og:updated_time, date
                    const dateMatch = text.match(/<meta[^>]+(?:name|property)=["'](?:article:published_time|article:modified_time|og:updated_time|date|citation_date|citation_publication_date)["'][^>]+content=["']([^"']+)["']/i);
                    if (dateMatch && dateMatch[1]) {
                        // Prefer the meta tag date often as it's the "semantic" publish date, not just server file time

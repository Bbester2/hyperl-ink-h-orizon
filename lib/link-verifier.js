 ...
 */
export async function verifyLink(url, context = '', options = {}) {
    const { strictMode = false, ignoredDomains = [] } = options;
    // Check cache first
    ...
    if (ignoredDomains.some(d => hostname.includes(d))) {
        result.status = 'working';
        result.reason = 'Ignored by settings';
        result.responseTime = Date.now() - startTime;
        cacheResult(url, result);
        return result;
    }
    ...
    if (headAttempt.response) {
        ...
        // HEAD returned 405 (Method Not Allowed) or 403 or 3xx (Redirect) → try GET
        else if (status === 405 || status === 403 || status === 400 || (status >= 300 && status < 400)) {
    ...
    if (finalResponse) {
        ...
        if (status >= 200 && status < 300) {
            if (restricted) {
                ...
                // Strict Mode: Flag redirects as "redirect" (Review) even if final status is 200
                if (strictMode && finalResponse.redirected) {
                    result.status = 'redirect';
                    result.reason = `Redirected to ${finalResponse.url}`;
                } else {
                    result.status = 'working';
                }
            }
            ...
            if (!strictMode) {
                result.status = 'working';
                result.reason = `Redirects to ${finalResponse.headers.get('location')}`;
            } else {
                result.status = 'redirect';
        ...
        // For GET responses with 200 status, do soft-404 check and date extraction
        if (result.status === 'working' && finalResponse.status === 200) {
            // 1. Try Last-Modified Header first
            const lastModified = finalResponse.headers.get('last-modified');
            if (lastModified) {
                result.date = new Date(lastModified).toISOString();
            }

            // If we don't have a date yet, and it's HTML, we might need to fetch the body
            // This is especially true if we only did a HEAD request so far
            let htmlText = '';
            const contentType = finalResponse.headers.get('content-type') || '';
            ...
            if (isHtml) {
                try {
                    try {
                        // If we already have the body (from a GET), use it
                        if (finalResponse.bodyUsed || usedMethod === 'GET' || finalResponse.url) {
                            // Note: 'finalResponse' might be from HEAD, so body extraction depends on how we fetched
                            // We can check if we successfully got text before, or if we need to fetch now.
                            // But 'finalResponse' stream might be locked/used.
                        }

                        // Simple logic: If we did a HEAD, we need to do a GET now to get metadata
                        // Only do this if we really strongly want the date and don't have it
                        if (!result.date) {
                            const getAttempt = await attemptFetch(url, 'GET', GET_TIMEOUT);
                            if (getAttempt.response && getAttempt.response.ok) {
                                htmlText = await getAttempt.response.text();
                            }
                        } else if (method === 'GET') {
                            // If we already did GET, we can read the body (if not read yet)
                            // But wait, above code struct implies 'finalResponse' was the successful one. 
                            // If we retried, 'finalResponse' is the good one.
                            try {
                                htmlText = await finalResponse.text();
                            } catch (e) { }
                        }
                    } catch (e) {
                        console.log('Error fetching body for metadata', e);
                    }

                    if (htmlText) {
                        if (isSoft404(htmlText)) {
                            result.status = 'broken';
                            result.reason = 'Soft 404 (page content indicates not found)';
                        }
                        if (PAYWALL_PATTERNS.some(pattern => pattern.test(htmlText))) {
                            result.status = 'restricted';
                            result.isRestricted = true;
                            result.reason = 'Paywall or login wall detected';
                        }

                        // 2. Try Meta Tags for Date
                        const dateMatch = htmlText.match(/<meta[^>]+(?:name|property)=["'](?:article:published_time|article:modified_time|og:updated_time|date|citation_date|citation_publication_date)["'][^>]+content=["']([^"']+)["']/i);
                        if (dateMatch && dateMatch[1]) {
                            try {
                                result.date = new Date(dateMatch[1]).toISOString();
                            } catch (e) { }
                        }
                    }
                }
     ...
     */
    export async function verifyLinks(links, options = {}) {
        ...
        const processNext = async () => {
            ...
            try {
                const result = await verifyLink(link.url, link.context, options);
                results.push({
                    ...result,

...
const SANITIZED_KEY = RAW_KEY.replace(/['"\s]/g, '');

// BLACKLIST: Explicitly ignore the old leaked key if it's still in Vercel env vars
const OLD_LEAKED_KEY = 'AIzaSyDAg6F4kJTVgaiWKSTM5q8kwbWqqMdTOo8';
const FINAL_KEY = (SANITIZED_KEY === OLD_LEAKED_KEY || SANITIZED_KEY.length < 10)
    ? 'AIzaSyBPQa6PqYxAWYNNVlA-H3-Zggw5iGins-k' // Fallback to new working key
    : SANITIZED_KEY;

const GEMINI_API_KEY = FINAL_KEY;
console.log('AI Service Init - Key Status:', GEMINI_API_KEY === OLD_LEAKED_KEY ? 'FAIL (Leaked)' : 'OK');

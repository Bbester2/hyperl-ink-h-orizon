...
const GEMINI_API_KEY = process.env.GEMINI_API_KEY || 'AIzaSyA4sz_KgI8nPWwftXcW2KcltL0edYpHZVg';
...
export async function getSuggestions(originalUrl, context, count = 3) {
    if (!GEMINI_API_KEY || GEMINI_API_KEY === 'your_gemini_api_key_here') {
        console.error("AI Key Check Failed. Key:", GEMINI_API_KEY ? "Present" : "Missing");
        const reason = !GEMINI_API_KEY ? "Key is literally undefined" : "Key is default placeholder";
        return getMockSuggestions(originalUrl, context, count, reason);
    }

...
export async function getSuggestions(originalUrl, context, count = 3) {
    if (!GEMINI_API_KEY) {
        const debugInfo = `Key Status: ${GEMINI_API_KEY === undefined ? 'Undefined' : 'Empty/Falsy'} [Type: ${typeof GEMINI_API_KEY}]`;
        console.error("AI Service Error:", debugInfo);
        return getMockSuggestions(originalUrl, context, count, debugInfo);
    }

...
console.log('AI Service Init - Key Configured:', GEMINI_API_KEY ? `Yes (Length: ${GEMINI_API_KEY.length})` : 'No');
...
async function quickVerifyUrl(url) {
    try {
        ...
        // On timeout/error, assume it *might* be valid but unverified
        // (Many sites block HEAD requests or automated tools)
        return null; // Return null to indicate "Unknown/Unverified" instead of false
    }
}
...
export async function getSuggestions(originalUrl, context, count = 3) {
    if (!GEMINI_API_KEY) {
        console.warn('AI Service: Missing API Key, returning mock suggestions.');
    ...
    try {
        ...
        if (suggestions.length > 0) {
            ...
            // Keep working URLs AND unverified ones (marked as such)
            // Only strictly remove confirmed dead links (if we implemented a strict check, which we haven't fully)

            suggestions = verificationResults.map(r => {
                const isVerified = r.isValid === true;
                const isUnknown = r.isValid === null;

                return {
                    ...r.suggestion,
                    verified: isVerified,
                    verificationStatus: isVerified ? 'verified' : (isUnknown ? 'unknown' : 'failed'),
                    // If unknown, add a warning to the description
                    description: isUnknown
                        ? `(Unverified) ${r.suggestion.description}`
